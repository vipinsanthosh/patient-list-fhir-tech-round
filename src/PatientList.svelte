...
<input on:keyup={(e) => debounce(e.target)} class="w-full border p-2" placeholder="Search by name or phone number (exact)">
...
{:then patientBundle} 
  {#each patientBundle?.entry || [] as patient}
    <p class="py-2 hover:bg-gray-200 px-1 cursor-pointer">
      ...
      {patient?.resource?.id}, {formatName(patient?.resource)} - {patient?.resource?.gender || ''} - {patient?.resource?.birthDate || ''}
      </a>
    </p>
  {/each}
{/await}
